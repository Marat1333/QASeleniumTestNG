...
public class SuppliesListPage extends CommonMagMobilePage {
    ...
    public SuppliesListPage shouldDataIsCorrect(ShipmentDataList data) {
        ...
        shipmentsUiData = shipmentsUiData.stream().distinct().collect(Collectors.toList());
        appointmentUiData = appointmentUiData.stream().distinct().collect(Collectors.toList());
        //
    ...
    public SuppliesListPage shouldWeekDataIsCorrect(ShipmentDataList... data) throws Exception {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (!needToVerify) {
                ...
                while (!anchor.isVisible()) mainScrollView.scrollToBeginning(1);
                anchor.waitForVisibility();
                dayOfWeekOption.click();
