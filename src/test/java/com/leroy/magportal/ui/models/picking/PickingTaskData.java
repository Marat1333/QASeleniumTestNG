...
public class PickingTaskData {
    ...
    public void assertEqualsNotNullExpectedFields(PickingTaskData expectedTaskData) {
        ...
        softAssert
                .isEquals(assemblyType, expectedTaskData.getAssemblyType(), "Неверный тип сборки");
        softAssert.isEquals(status.toLowerCase(), expectedTaskData.getStatus().toLowerCase(),
                "Неверный статус");
        if (expectedTaskData.getCreationDate() != null) {
            softAssert.isEquals(creationDate, expectedTaskData.getCreationDate(),
                    "Неверная дата создания сборки");
        }
        softAssert.isEquals(products.size(), expectedTaskData.getProducts().size(),
                "Неверное кол-во товаров");
