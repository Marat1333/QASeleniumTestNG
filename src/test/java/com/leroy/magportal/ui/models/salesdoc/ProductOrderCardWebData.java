...
public class ProductOrderCardWebData {
    ...
    public void setDiscountPercent(Double discountPercent, boolean reCalculate) {
        ...
        if (reCalculate) {
            this.totalPriceWithDiscount = ParserUtil
                    .minus(totalPrice, (totalPrice * discountPercent / 100), 2);
        }
    }

    public void assertEqualsNotNullExpectedFields(ProductOrderCardWebData expectedProduct,
            int iOrder, int iProduct) {
        ...
        if (expectedProduct.getSelectedQuantity() != null) {
            softAssert.isEquals(this.getSelectedQuantity(), expectedProduct.getSelectedQuantity(),
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидался другое кол-во");
        }
        if (expectedProduct.getTotalPrice() != null) {
            softAssert.isEquals(this.getTotalPrice(), expectedProduct.getTotalPrice(),
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидалась другая стоимость");
        }
        if (expectedProduct.getTotalPriceWithDiscount() != null) {
            softAssert.isEquals(this.getTotalPriceWithDiscount(),
                    expectedProduct.getTotalPriceWithDiscount(),
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидалась другая стоимость (с учетом скидки)");
        }
        if (expectedProduct.getDiscountPercent() != null) {
            softAssert.isEquals(this.getDiscountPercent(), expectedProduct.getDiscountPercent(),
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидалась другая скидка %");
        }
        if (expectedProduct.getAvailableTodayQuantity() != null && !DefectConst.STOCK_ISSUE) {
            softAssert.isEquals(this.getAvailableTodayQuantity(),
                    expectedProduct.getAvailableTodayQuantity(),
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидалось другое доступное кол-во");
        } else if (this.getAvailableTodayQuantity() != null) {
            softAssert.isTrue(this.getAvailableTodayQuantity() >= 0,
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидалось, что доступное кол-во >= 0");
        }
        if (expectedProduct.getWeight() != null) {
            softAssert.isTrue(Math.abs(this.getWeight() - expectedProduct.getWeight())
                            <= expectedProduct.getSelectedQuantity() * 0.011,
                    "Заказ #" + (iOrder + 1) + " Товар #" + (iProduct + 1)
                            + " - ожидался другой вес. " +
                            "Актуальный:" + this.getWeight() + " Ожидался:" + expectedProduct
                            .getWeight());
        } else {
