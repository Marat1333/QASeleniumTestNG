...
public class SalesDocWebData {
    ...
    public boolean isDocumentContainsProduct(String lmCode) {
        for (OrderWebData orderWebData : orders) {
            for (ProductOrderCardWebData productOrderCardWebData : orderWebData
    ...
    public ShortOrderDocWebData getShortOrderData() {
        ...
        if (creationDate != null) {
            shortOrderDocWebData.setCreationDate(
                    DateTimeUtil.strToLocalDateTime(creationDate, "dd MMM, HH:mm"));
        }
        shortOrderDocWebData
                .setDeliveryType(deliveryType.equals(SalesDocumentsConst.GiveAwayPoints.PICKUP) ?
                        OrderConst.DeliveryType.PICKUP
                        : OrderConst.DeliveryType.DELIVERY_TK); // todo
    ...
    public void assertEqualsNotNullExpectedFields(SalesDocWebData expectedData) {
        ...
        if (expectedData.getStatus() != null) {
            softAssert.isEquals(this.getStatus().toLowerCase(),
                    expectedData.getStatus().toLowerCase(),
        ...
        if (expectedData.getOrders() != null) {
            ...
            for (int i = 0; i < expectedData.getOrders().size(); i++) {
                this.getOrders().get(i)
                        .assertEqualsNotNullExpectedFields(expectedData.getOrders().get(i), i);
            }
        }
