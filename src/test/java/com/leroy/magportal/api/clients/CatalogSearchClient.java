...
public class CatalogSearchClient extends com.leroy.magmobile.api.clients.CatalogSearchClient {
    ...
    public List<ProductItemData> getRandomUniqueProductsWithTitlesForShop(int countOfProducts,
        ...
        ProductItemDataList productItemDataList = this
                .searchProductsBy(new GetCatalogSearch().setPageSize(MAX_PAGE_SIZE)
                        .setHasAvailableStock(true).setShopId(shopId)).asJson();
        List<ProductItemData> productItemData = productItemDataList.getItems();
        productItemData = productItemData.stream().filter(i -> i.getTitle() != null)
